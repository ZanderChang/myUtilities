...
### DRILLING
    ...
    def _drill_input(self):
        ...
        r = tracer.qemu_runner.QEMURunner(self.binary, self.input, argv=self.argv) # QEMU执行一次
        ...
        if p.loader.main_object.os == 'cgc':
            ...
            s = p.factory.full_init_state(stdin=angr.SimFileStream, mode='tracing') # 创建state

        s.preconstrainer.preconstrain_file(self.input, s.posix.stdin, True) # 为stdin添加初始约束
        ...
        while simgr.active and simgr.one_active.globals['trace_idx'] < len(r.trace) - 1:
            ...
            if 'diverted' not in simgr.stashes: # DrillerCore未找到结果
                continue
...
### UTILS
    ...
    def _writeout(self, prev_addr, state):
        generated = state.posix.stdin.load(0, state.posix.stdin.pos) # 获取state的stdin的bitvector表示
        generated = state.solver.eval(generated, cast_to=bytes) # 在此处求解
